...
clientController.loadClientSignupPage = (req, res, next) => {
  ...
  try{
    if(!isLoggedIn){
      ...
      return res.status(302).redirect(`/client/home?username=${encodeURIComponent(username)}&message=${encodeURIComponent("***you're already logged in!")}`);
    }
  }catch(err){
    next(err.message);
  }
...
clientController.loadClientLoginPage = (req, res, next) => {
  ...
  try{
    if(!isLoggedIn){
      ...
      return res.status(303).redirect(`/client/home?username=${encodeURIComponent(username)}&message=${encodeURIComponent("***you're already logged in!")}`)
    }
  }catch(err){
    next(err.message);
  }
  ...
  try{
    ...
    if(typeof getResult === "string"){
      ...
      if(isPswMatched){
        ...
        return res.status(302).redirect(`/client/home?message=${encodeURIComponent("***log in successful!")}`);
      }else{
        return res.status(303).redirect(`/client/login?message=${encodeURIComponent("***log in failed, invalid credentials!")}`);
      }
    }
  }catch(err){
    next(err.message);
...
clientController.clientLogout = (req, res, next) => {
  ...
  try{
    if(username.length > 0 && userType === "client"){
      ...
      return res.status(302).redirect(`/client/login?message=${encodeURIComponent("***you're successfully logged out!")}`);
    }else{
      return res.status(303).redirect(`/client/home?message=${encodeURIComponent("***something went wrong, unable to log out!")}`);
    }
  }catch(err){
    next(err.message);
  }
